...
class CanvasAndDataFrame(ttk.Frame):
    def __init__(self, container, image, study):
        super().__init__(container)
        # -- IMAGE DATA FRAME
        self.study = study
        self.image = image
        self.grid_rowconfigure(0, weight=1)
        self.image_data_frame = ImageDataFrame(self, study, image, study.patient_name)
        self.image_data_frame.grid(row=0, column=1, sticky='NSEW', padx=10, pady=5)
        # --
        appdata = None
        # -- PHOTO_FRAME
        # ---- LABEL_MAIN_PHOTO
        self.canvas = MyCanvas(self, appdata, study, image)
...
class ImageDataFrame(ttk.Frame):
...
class MyCanvas(tk.Canvas):
    def __init__(self, container, appdata, study, image: ZImage):
    ...
    def _print_roi_from_points_interpolated(self):
